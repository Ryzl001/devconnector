...
// @route   GET api/profile/handle/:handle  - to jest backend API - z front endu to będzie api/profile/nazwaUsera
...
router.get('/handle/:handle', (req, res) => {
    ...
    Profile.findOne({ handle: req.params.handle })
        ...
        .then(profile => {
            if(!profile) {
                errors.noprofile = 'There is no progile for this user';
                res.status(404).json(errors);
            }
...
router.delete('/experience/:exp_id', passport.authenticate('jwt', { session: false }), (req, res) => {
    ...
    Profile.findOne({ user: req.user.id })  //user id przychodzi z wygenerowanego tokena
        .then(profile => {
            ...
            .map(item => item.id) // tworzymy tablicę z samymi id experience
            .indexOf(req.params.exp_id)  // zwracamy indeks który zawiera exp_id

            // Splice out of array
            profile.experience.splice(removeIndex, 1); // usuwamy experience o wcześniej określonym indeksie
...
router.delete('/education/:edu_id', passport.authenticate('jwt', { session: false }), (req, res) => {
    ...
    Profile.findOne({ user: req.user.id })  //user id przychodzi z wygenerowanego tokena
        .then(profile => {
            ...
            .map(item => item.id) // tworzymy tablicę z samymi id education
            .indexOf(req.params.edu_id)  // zwracamy indeks który zawiera edu_id

            // Splice out of array
            profile.education.splice(removeIndex, 1); // usuwamy education o wcześniej określonym indeksie
